...
class TaskSubtask(models.Model):
    ...
    def _check_subtask_deadline(self):
        ...
        for subtask in self:
            if subtask.deadline and subtask.parent_task_id.date_deadline:
                parent_deadline_date = subtask.parent_task_id.date_deadline.date() if isinstance(
                    subtask.parent_task_id.date_deadline, fields.Datetime
                ) else subtask.parent_task_id.date_deadline
                
                if subtask.deadline > parent_deadline_date:
    ...
    def _onchange_deadline(self):
        ...
        if self.deadline and self.parent_task_id.date_deadline:
            parent_deadline_date = self.parent_task_id.date_deadline.date() if isinstance(
                self.parent_task_id.date_deadline, fields.Datetime
            ) else self.parent_task_id.date_deadline
            
            if self.deadline > parent_deadline_date:
                return {
                    'warning': {
